...
export async function buildAppointmentCompletedMessage(
  apt: AppointmentWithDetails,
  locale: Locale,
): Promise<RenderedMessage> {
  const dict = await getDictionary(locale, 'staffNotifications') as Record<string, unknown>;
  const completed = dict.completed as Record<string, string>;
  ...
  const subject = interpolate(completed.subject, interp);
