...
function buildPlainText(apt: AppointmentWithDetails): string {
  ...
  return [
    `✅ ${locationType} booking confirmed`,
    `Name: ${customerName}`,
...
function buildHtml(apt: AppointmentWithDetails): string {
  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #f5f5f5;">
      <div style="background: #ffffff; border-radius: 8px; padding: 24px; border: 1px solid #e0e0e0; border-left: 4px solid #22c55e;">
        <h2 style="margin: 0 0 16px; color: #1a1a1a; font-size: 20px;">✅ ${locationType} Booking Confirmed</h2>
...
function buildTelegramHtml(apt: AppointmentWithDetails): string {
  ...
  const lines = [
    `<b>✅ ${locationType} booking confirmed</b>`,
