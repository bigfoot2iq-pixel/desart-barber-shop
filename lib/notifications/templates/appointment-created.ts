...
export async function buildAppointmentCreatedMessage(
  apt: AppointmentWithDetails,
  locale: Locale,
): Promise<RenderedMessage> {
  const dict = await getDictionary(locale, 'staffNotifications') as Record<string, unknown>;
  const created = dict.created as Record<string, string>;
  ...
  const plainText = [
    `🆕 New ${locationType} — ${locale === 'fr' ? 'En attente' : 'pending'}`,
  ...
  const telegramLines = [
    `<b>🆕 New ${locationType} — ${locale === 'fr' ? 'En attente' : 'pending'}</b>`,
