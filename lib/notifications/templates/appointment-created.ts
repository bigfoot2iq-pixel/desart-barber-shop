...
function buildPlainText(apt: AppointmentWithDetails): string {
  ...
  return [
    `🆕 New ${locationType} booking — pending`,
    `Name: ${customerName}`,
  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #f5f5f5;">
      <div style="background: #ffffff; border-radius: 8px; padding: 24px; border: 1px solid #e0e0e0;">
        <h2 style="margin: 0 0 16px; color: #1a1a1a; font-size: 20px;">🆕 New ${locationType} Booking — Pending</h2>
...
function buildTelegramHtml(apt: AppointmentWithDetails): string {
  ...
  const lines = [
    `<b>🆕 New ${locationType} booking — pending</b>`,
...
export function buildAppointmentCreatedMessage(apt: AppointmentWithDetails): RenderedMessage {
  return {
    subject: `New booking — ${apt.customer?.first_name ?? 'Customer'} • ${formatDate(apt.appointment_date)}`,
