...
function buildPlainText(apt: AppointmentWithDetails): string {
  ...
  return [
    `❌ ${locationType} booking cancelled`,
    `Name: ${customerName}`,
  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #f5f5f5;">
      <div style="background: #ffffff; border-radius: 8px; padding: 24px; border: 1px solid #e0e0e0; border-left: 4px solid #ef4444;">
        <h2 style="margin: 0 0 16px; color: #1a1a1a; font-size: 20px;">❌ ${locationType} Booking Cancelled</h2>
...
function buildTelegramHtml(apt: AppointmentWithDetails): string {
  ...
  const lines = [
    `<b>❌ ${locationType} booking cancelled</b>`,
...
export function buildAppointmentCancelledMessage(apt: AppointmentWithDetails): RenderedMessage {
  const customerName = `${apt.customer?.first_name ?? ''} ${apt.customer?.last_name ?? ''}`.trim();
  const services = apt.services.map((s) => s.name).join(', ');
  const locationType = apt.location_type === 'home' ? 'Come to Home' : 'Salon';
  return {
    subject: `Booking cancelled — ${customerName} • ${formatDate(apt.appointment_date)}`,
