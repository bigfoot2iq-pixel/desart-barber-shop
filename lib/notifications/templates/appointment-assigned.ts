...
export async function buildAppointmentAssignedMessage(
  apt: AppointmentWithDetails,
  locale: Locale,
): Promise<RenderedMessage> {
  const dict = await getDictionary(locale, 'staffNotifications') as Record<string, unknown>;
  const assigned = dict.assigned as Record<string, string>;
  ...
  const interp = { customerName, date: dateStr, locationType, professionalName };
  const subject = interpolate(assigned.subject, interp);
  ...
  const plainText = [
    ...
    `${dict.professionalLabel as string}: ${professionalName}`,
  ...
  const html = `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #f5f5f5;">
      <div style="background: #ffffff; border-radius: 8px; padding: 24px; border: 1px solid #e0e0e0; border-left: 4px solid #a855f7;">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 14px; color: #333;">
          ...
          <tr><td style="padding: 6px 0; color: #888;">${dict.professionalLabel as string}</td><td style="padding: 6px 0; font-weight: 600;">${professionalName}</td></tr>
  ...
  const telegramLines = [
    ...
    `<b>${dict.professionalLabel as string}:</b> ${professionalName}`,
