...
function buildPlainText(apt: AppointmentWithDetails): string {
  ...
  return [
    `👤 Professional assigned — ${locationType}`,
    `Name: ${customerName}`,
  ...
  return `
    <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #f5f5f5;">
      <div style="background: #ffffff; border-radius: 8px; padding: 24px; border: 1px solid #e0e0e0; border-left: 4px solid #a855f7;">
        <h2 style="margin: 0 0 16px; color: #1a1a1a; font-size: 20px;">👤 Professional Assigned — ${locationType}</h2>
        <table style="width: 100%; border-collapse: collapse; font-size: 14px; color: #333;">
          ...
          <tr><td style="padding: 6px 0; color: #888;">Professional</td><td style="padding: 6px 0; font-weight: 600;">${professionalName}</td></tr>
...
function buildTelegramHtml(apt: AppointmentWithDetails): string {
  ...
  const lines = [
    `<b>👤 Professional assigned — ${locationType}</b>`,
