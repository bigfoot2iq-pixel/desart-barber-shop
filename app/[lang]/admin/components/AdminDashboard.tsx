...
import { getPendingAppointments, getTodayAppointmentsCount, getActiveProfessionalsCount, getActiveServicesCount, getAllAppointments } from '@/lib/queries';
...
export default function AdminDashboard({ lang, initialPendingCount, adminName, adminEmail }: AdminDashboardProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [pendingApts, allApts, todayCnt, profsCount, servsCount] = await Promise.all([
  ...
  const quickLinks = [
    { section: 'appointments' as const, icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z' },
    { section: 'professionals' as const, icon: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
    { section: 'services' as const, icon: 'M14.121 14.121L19 19m-4.879-4.879l-2.652 2.652a3 3 0 01-4.243 0l-.59-.59a3 3 0 010-4.243l2.652-2.652m4.833 4.833L9.9 9.9m4.833 4.833l2.652-2.652a3 3 0 000-4.243l-.59-.59a3 3 0 00-4.243 0l-2.652 2.652' },
    { section: 'salons' as const, icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4' },
    { section: 'notifications' as const, icon: 'M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9' },
    { section: 'payment' as const, icon: 'M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z' },
  ];
    ...
    >
      <div className="space-y-8">
        ...
        <div>
          ...
          {recentPending.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentPending.map((apt) => (
                <Link key={apt.id} href={localeHref(lang, '/admin/appointments')}>
          ...
          {pendingCount > 0 && (
            <Link href={localeHref(lang, '/admin/appointments')}>
              <Button
                variant="ghost"
                size="sm"
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {quickLinks.map(({ section, icon }) => (
            <Link key={section} href={localeHref(lang, `/admin/${section}`)}>
              <Card className="cursor-pointer group hover:border-primary/30 transition-colors h-full">
                <CardContent className="p-4">
